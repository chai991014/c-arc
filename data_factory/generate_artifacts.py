...
def extract_dwas_and_tasks(conn):
    print("⏳ Extracting DWAs and Tasks (The Action Dictionaries)...")

    # Extract DWAs
    dwa_df = pd.read_sql_query(
        "SELECT DISTINCT dwa_id as id, dwa_title as name FROM dwa_reference WHERE dwa_id IS NOT NULL", conn)
    with open(os.path.join(ARTIFACT_DIR, "dwa_pool.json"), 'w', encoding='utf-8') as f:
        json.dump(dwa_df.to_dict('records'), f, indent=4)

    # Extract Tasks
    task_df = pd.read_sql_query(
        "SELECT DISTINCT task_id as id, task as name FROM task_statements WHERE task_id IS NOT NULL", conn)
    with open(os.path.join(ARTIFACT_DIR, "task_pool.json"), 'w', encoding='utf-8') as f:
        json.dump(task_df.to_dict('records'), f, indent=4)

    print(f"   ↳ ✅ Success: Saved DWA and Task pools.")
...
def main():
    ...
    print("-" * 40)
    extract_dwas_and_tasks(conn)

    conn.close()
    print("\n🎉 Artifact generation is officially 100% COMPLETE.")
