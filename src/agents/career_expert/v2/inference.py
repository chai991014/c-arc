...
class CareerExpert:
    def __init__(self,
                 model_dir="./agents/career_expert/assets",
        ...
        self.knn = joblib.load(f"{model_dir}/knn_retrieval_index.joblib")
        with open(f"{model_dir}/benchmark_matrix.pkl", 'rb') as f:
            self.benchmark_matrix = pickle.load(f)
        with open(f"{model_dir}/soc_index_map.pkl", 'rb') as f:
            self.soc_map = pickle.load(f)

        # 2. Load Vectorizer
        with open(f"{model_dir}/feature_vectorizer.pkl", 'rb') as f:
            self.vec = pickle.load(f)

        # 3. Load Stage 2 (Scoring Regressor)
        self.model = xgb.XGBRegressor()
        self.model.load_model(f"{model_dir}/career_expert_xgboost_regressor.json")
    ...
    def _parse_user_profile(self, master_profile):
        ...
        for tech in master_profile.get("tech_skills", []):
            ...
            if tech_id.startswith("TECH-"):
                tech_id = tech_id.replace("TECH-", "")
    ...
    def predict(self, master_profile: dict, ocean_vector: dict):
        ...
        distances, indices = self.knn.kneighbors(user_sparse, n_neighbors=15)
        ...
        # Stack into a single prediction matrix
        X_inference = vstack(inference_rows).tocsr()
        print(f"\n  • Synthesized Matrix Shape for Regressor: {X_inference.shape}")

        # 4. Predict Absolute Capability Scores
        predicted_scores = self.model.predict(X_inference)
