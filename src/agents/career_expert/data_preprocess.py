...
def build_pipeline_assets():
    ...
    # APPLY TF-IDF TO THE BENCHMARK MATRIX RIGHT BEFORE SAVING ---
    print("Transforming benchmark matrix with TF-IDF...")
    tfidf = TfidfTransformer(norm=None)
    tfidf_benchmark_matrix = tfidf.fit_transform(benchmark_matrix)
    ...
    X_train, X_temp, y_train, y_temp, df_syn_train, df_syn_temp = train_test_split(
        X_full, y_full, df_syn, test_size=0.30, random_state=42
    )
    X_test, X_val, y_test, y_val, df_syn_test, df_syn_val = train_test_split(
        X_temp, y_temp, df_syn_temp, test_size=0.5, random_state=42
    )
    ...
    with open(os.path.join(EXPORT_DIR, "tfidf_transformer.pkl"), 'wb') as f:
        pickle.dump(tfidf, f)
    with open(os.path.join(EXPORT_DIR, "tfidf_benchmark_matrix.pkl"), 'wb') as f:
        pickle.dump(tfidf_benchmark_matrix, f)
    ...
    train_csv_path = os.path.join(EXPORT_DIR, "train_dataset.csv")
    df_syn_train.to_csv(train_csv_path, index=False)
    val_csv_path = os.path.join(EXPORT_DIR, "val_dataset.csv")
    df_syn_val.to_csv(val_csv_path, index=False)
    test_csv_path = os.path.join(EXPORT_DIR, "test_dataset.csv")
    df_syn_test.to_csv(test_csv_path, index=False)
    print(f"📦 Successfully saved {len(df_syn_test)} raw test rows to {test_csv_path}")
