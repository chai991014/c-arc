...
class CareerExpert:
    ...
    def predict(self, master_profile: dict, ocean_vector: dict, verbose=True):
        # 1. Format User Data
        user_features = self._parse_user_profile(master_profile)
        user_sparse = self.vec.transform([user_features])
        if verbose: print(f"  • Parsed Vector Features : {len(user_features)} active entries mapped to sparse space.")
        if len(user_features) > 0:
            if verbose: print(f"    ↳ Active features: {list(user_features.keys())[:8]}...")
        ...
        if verbose: print(f"  • Reconciled Vector OCEAN: {user_ocean.tolist()}")

        # Pre-cache user constants for acceleration across full-space looping
        ...
        # 2. Full-Space Inference Matrix Construction
        inference_rows = []
        candidate_socs = []
        total_jobs = self.benchmark_matrix.shape[0]
        ...
        for idx in range(total_jobs):
            ...
            # Interaction Features matching your exact implementation logic
        ...
        # 3. Predict Absolute Capability Scores using Neural Network
        ...
        # Sort all targets globally by match ranking
        results = sorted(results, key=lambda x: x["match_score"], reverse=True)

        if verbose: print("\n🔮 FULL-SPACE MATCHING (DCN V2 Scorer):")
        for rank, res in enumerate(results[:50]):
            if verbose: print(
